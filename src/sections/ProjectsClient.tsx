...
import { capitalizeWords, getPlaceholderUrl } from '@/lib/utils';
...
export async function getProjectThumbnail(owner: string, repo: string): Promise<string> {
            ...
            <div className="container mx-auto px-4">
                ...
                <div className='flex flex-col md:flex-row gap-6 justify-center items-center md:flex-wrap my-8'>
                    {projects.slice(0, visibleItems).map((project) => (
                        <Link key={project.id} href={`/projects/${project.name}/`}>
                            <div className="hovser:scale-105 overflow-hidden transition-all duration-1000 relative group h-[330px] w-full md:aspect-auto md:w-[300px] lg:w-[366px] md:h-[330px] rounded-2xl">
                    
                                <Image
                                   src={projectThumbnails[project.id] || getPlaceholderUrl(project) }
                ...
                {page === 'home' && (
                    <div className="flex items-center">
                        <ViewAllButton disabled={!projects.length} to='/projects' />
                    </div>
                )}
